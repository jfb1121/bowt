...
func runSupervisor(ls state.LaneStore, ag agent.Agent, spec spawn.LaneSpec, acquire func(string) (*lock.Lock, error), stdout, stderr io.Writer) error {
	...
	status, err := spawn.TerminalStatus(spec.Mode, exitCode, filepath.Join(spec.Worktree, spec.WritebackDir))
	if err != nil {
		return err
	}
	lane.Status = status
...
func laneFromSpec(spec spawn.LaneSpec) state.Lane {
	status := state.StatusPlanning
	if spec.Mode == string(spawn.ModeImpl) {
		status = state.StatusImpl
	}
	return state.Lane{
		ID:            spec.ID,
		Ticket:        spec.Ticket,
